...
- Vendas (ex-"Pedidos": grid de cards com badge de origem, Bruto/Custos/Líquido, filtro de status + canal de origem, overlay com barra de status **só avança** exceto pelo botão "Cancelar Venda" que joga pro status `cancelled` a qualquer momento, doc. de envio em PDF). Modal "Nova Venda Manual" (`NewSaleModal.tsx`) tem seção opcional "Filamento(s) Utilizados" (só na criação, não na edição) — ao salvar, desconta `remaining_weight_g` de cada filamento selecionado e cria o `filament_movements` de `sale_consumption`; se a quantidade informada passar do disponível só avisa, não bloqueia a venda. A constraint `quotes_status_check` no banco não incluía `'cancelled'` (só tinha `sent/paid/in_production/shipped/expired`) — corrigido via migration em 2026-08-19; antes disso, cancelar uma venda atualizava o status só na UI (otimista) e o `UPDATE` no Supabase falhava silenciosamente, por isso a venda cancelada nunca aparecia em Financeiro (que lê direto do banco). `handleStatusChange` em `app/dashboard/orders/page.tsx` agora trata erro do `update` e reverte o estado local + avisa o usuário se falhar de novo. "Gerar Comprovante" (`SaleReceiptModal.tsx`): gera a imagem automaticamente ao abrir, mas os dois botões finais ("Salvar" e "Compartilhar no WhatsApp") são sempre acionados manualmente — nada baixa nem abre nada sozinho. "Compartilhar no WhatsApp" só existe se a venda tem cliente cadastrado com WhatsApp; ao clicar, baixa a imagem e abre a conversa do cliente (`wa.me`) com mensagem pronta na mesma ação — a API do WhatsApp não permite anexar arquivo via link, então o usuário só precisa anexar a imagem que já caiu nos downloads.
- Estoque (`/dashboard/inventory`, título só "Estoque" — a aba "Histórico de Vendas" foi removida de lá; produtos físicos prontos/pronta-entrega — **não** confundir com Filamentos, são telas e tabelas diferentes que coexistem de propósito): Adicionar Estoque + Registrar Venda direto na listagem
...
- Financeiro (`/dashboard/finance`): KPIs de Receita Bruta/Custos Totais/Lucro Líquido Real/Vendas Canceladas, filtro de período+origem, gráfico de evolução, despesas via `extra_purchases` (botão "Nova Despesa"), exportar CSV e gerar relatório PDF, seção "Cupons e Descontos" (`CouponsPeriodSummary.tsx`, movida do Dashboard pra cá) + "Cupons e Campanhas" (`CouponsCampaignsSection.tsx`) — tudo com dados reais.
- Suporte (`/dashboard/support`): WhatsApp direto pra quem é pago; plano Grátis cai numa Central de Ajuda (`/help`, FAQ estática, pública e sem exigir login). "Sugestões e Reclamações" (`/feedback`, também público) aceita envio de visitante deslogado — nesse caso pede nome/e-mail manual (`guest_name`/`guest_email`, `user_id` fica `null`); logado, pré-preenche a partir do perfil e usa `user_id`.
...
7. A MCP do Supabase desconectou no meio da sessão uma vez — nesse caso, migrations (ex: adicionar `cancelled`/`mercado_livre` aos check constraints de `quotes`/`integrations`) tiveram que ser entregues como SQL pra colar manualmente no SQL Editor do painel, em vez de aplicadas direto. Se voltar a acontecer, confirme com o usuário se prefere reconectar a MCP ou receber o SQL. **`cancelled` nunca chegou a entrar de fato no `quotes_status_check`** (conferido direto no banco em 2026-08-19 — a constraint só tinha `sent/paid/in_production/shipped/expired`) — o SQL manual dessa vez provavelmente não foi colado, ou foi revertido. Reaplicado via MCP nessa data; se o mesmo tipo de sintoma aparecer de novo (mudança de status que não reflete em outra tela), **confira a constraint real no banco antes de assumir bug de frontend**.
8. Cotação de frete (`lib/melhorEnvio.ts`) só retorna Correios se a conta do Melhor Envio conectada em Integrações for de **produção** com contrato Correios habilitado no painel deles — no ambiente sandbox (`MELHOR_ENVIO_ENV` não setado como `production`), só transportadoras de teste tipo Jadlog respondem cotação real, Correios não aparece por padrão. Isso não é bug de código. `calculateShipping` agora retorna `unavailable` (motivo por transportadora que não cotou) além de `quotes`, exibido no `ShippingQuoteWidget` — usar isso pra diagnosticar em vez de assumir.
